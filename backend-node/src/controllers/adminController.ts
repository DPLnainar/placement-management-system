                        ...
                        await sendEmail({
                            ...
                            html: `
                                ...
                                <ul>
                                    ...
                                    <li><strong>Package:</strong> ${job.packageLPA ? `₹${job.packageLPA} LPA` : job.salary || 'Not specified'}</li>
...
/**
 * POST /api/admin/jobs
 * Create a new job with enhanced eligibility criteria and auto-notification
 */
export const createJob = async (req: IAuthRequest, res: Response): Promise<void> => {
    try {
        ...
        // Validate required fields
        if (!jobData.title || !jobData.companyName || !jobData.role) {
            res.status(400).json({
                success: false,
        ...
        if (job.status === 'active') {
            try {
                        ...
                        if (userEmail) {
                            try {
                                const packageInfo = job.category?.includes('intern')
                                    ? `₹${job.stipend}/month for ${job.durationMonths} months`
                                    : `₹${job.packageLPA} LPA`;
                                ...
                                await sendEmail({
                                    ...
                                    html: `
                                        ...
                                        <ul>
                                            <li><strong>Company:</strong> ${job.companyName}</li>
                                            <li><strong>Position:</strong> ${job.title}</li>
                                            <li><strong>Category:</strong> ${job.category?.join(', ')}</li>
                                            <li><strong>Package:</strong> ${packageInfo}</li>
                ...
                console.log(`Job created. Notified ${notifiedCount} eligible students.`);
            } catch (notifyError) {
