 ...
 */
export const welcomeEmail = ({ fullName, username, role, collegeName }: any): string => {
    const roleNames: any = {
        student: 'Student',
        moderator: 'Moderator',
        admin: 'Administrator'
 ...
 */
export const invitationEmail = ({ fullName, email, collegeName, invitationToken, expiresAt }: any): string => {
 ...
 */
export const passwordResetEmail = ({ fullName, resetToken, expiresIn = '1 hour' }: any): string => {
 ...
 */
export const applicationSubmittedEmail = ({ fullName, jobTitle, company, applicationDate }: any): string => {
    const content = `
 ...
 */
export const offerLetterEmail = ({ fullName, jobTitle, company, package: pkg, joiningDate }: any): string => {
    const content = `
 ...
 */
export const bulkUploadSummaryEmail = (results: any, superAdminName: string, superAdminEmail: string): string => {
              ...
              <tr style="border-bottom: 1px solid #ddd;">
                ...
                <td style="padding: 10px; border: 1px solid #ddd;">
                  <ul style="margin: 0; padding-left: 20px; color: #721c24;">
                    ${item.errors.map((error: any) => `<li>${error}</li>`).join('')}
                  </ul>
                </td>
              </tr>
            `).join('')}
